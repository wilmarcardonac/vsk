...
!   use fitstools, only: output_map
  ...
  integer(I4B) :: i,j,l, nm_eff1, nm_eff2, nm_eff, k1, k2
...
!!!!EH  if (correct_beam.and..not.beam_present) allocate(gb(0:nlmax,1))
  ...
  allocate(mu(1:nlmax+1),w(1:nlmax+1),Pl(0:nlmax,0:nlmax,0:ncor-1))
...
!  call do_legendre(nside,nlmax,mumin,mu,w,Pl,megaverbose,ncor) 
     ...
     if (map_present) then
        ...
        if (degree_remove == 1 .or. degree_remove == 2) then
           ...
           do j=1, 1 ! nmap  2014-03-27: turn-off monopole and dipole removal for Q,U maps
              call remove_dipole(nside, map_in(:,j), 1, degree_remove, lowmultipoles, zbounds, silent=.not.megaverbose)
           enddo
        endif
        ...
        if (map2_present) then
           ...
           if (degree_remove == 1 .or. degree_remove == 2) then
              ...
              do j=1, 1 ! nmap  2014-03-27: turn-off monopole and dipole removal for Q,U maps
                 call remove_dipole(nside, map2_in(:,j), 1, degree_remove, lowmultipoles, zbounds, silent=.not.megaverbose)
              enddo
           endif
           ! Compute the cross-Cls of the maps
           call do_cl_with_alm(nside,npixtot,nlmax,map_in,cl_final, &
                &                        megaverbose,'(mapXmap)',nmap,ncor,polarization, &
                &                        map2=map2_in, cl_auto1=cl_auto1, cl_auto2=cl_auto2, symmetric=symmetric_cl)
        else
...
&                  '(map)',thetamax,mask_correction)
 ...
 &                             apodizesigma=apodizesigma,thetamax=thetamax)
  ...
  elseif (mask_correction) then
     ...
     allocate(m_map(0:npixtot-1,1:nm_eff))
     m_map = 1.0
     if (masks_present) then
        ! convert mask to RING if necessary
        if (ordering_m1 == 2) call convert_nest2ring(nside, mask_map)
        ! apply mask
        do j=1, nm_eff
           kmask = min(j, nmask1)
           do i=0, npixtot-1
              m_map(i,j) = m_map(i,j) * mask_map(i,kmask)
           enddo
        enddo
        deallocate(mask_map)
     endif
     if (weights_present) then
        ! convert weight to RING if necessary
        if (ordering_w1 == 2) call convert_nest2ring(nside, weight_map)
        ! apply weight
        do j=1, nm_eff
           kmask = min(j, nweight1)
           do i=0, npixtot-1
              m_map(i,j) = m_map(i,j) * weight_map(i,kmask)
           enddo
        enddo
        deallocate(weight_map)
     endif

     if (map2_present) then ! second map -> second mask required
        allocate(m_map2(0:npixtot-1,1:nm_eff))
        m_map2 = 1.0 ! full sky
        if (do_mask2) then ! either mask2 or weight2 provided by user
           ...
           if (masks2_present) then
              ! convert mask to RING if necessary
              if (ordering_m2 == 2) call convert_nest2ring(nside, mask2_map)
              ! apply mask
              do j=1, nm_eff
                 kmask2 = min(j, nmask2)
                 do i=0, npixtot-1
                    m_map2(i,j) = m_map2(i,j) * mask2_map(i,kmask2)
                 enddo
              enddo
              deallocate(mask2_map)
           endif
           if (weights2_present) then
              ! convert weight to RING if necessary
              if (ordering_w2 == 2) call convert_nest2ring(nside, weight2_map)
              ! apply weight
              do j=1, nm_eff
                 kmask2 = min(j, nweight2)
                 do i=0, npixtot-1
                    !!! m_map2(i,j) = m_map2(i,j) * weight2_map(i,j) ! corrected 2014-04-03
                    m_map2(i,j) = m_map2(i,j) * weight2_map(i,kmask2)
                 enddo
              enddo
              deallocate(weight2_map)
           endif
        else
     ...
     if (weights_present) then
        if (nm_eff > 1 .and. weightpower /= weightpowerp) then
           call apply_w8_power(m_map(:,1:1), npixtot, 1, weightpower,  megaverbose, '(temp. 1)')
           call apply_w8_power(m_map(:,2:2), npixtot, 1, weightpowerp, megaverbose, '(pol. 1)')
        else
           call apply_w8_power(m_map, npixtot, nm_eff, weightpower, megaverbose, '(map 1)')
        endif
     endif
     if (map2_present .and. weights2_present) then
        if (nm_eff > 1 .and. weightpower2 /= weightpowerp2) then
           call apply_w8_power(m_map2(:,1:1), npixtot, 1, weightpower2,  megaverbose, '(temp. 2)')
           call apply_w8_power(m_map2(:,2:2), npixtot, 1, weightpowerp2, megaverbose, '(pol. 2)')
        else
           call apply_w8_power(m_map2, npixtot, nm_eff, weightpower2, megaverbose, '(map 2)')
        endif
     endif
        
     ! --- check that the 2 mask*weights are not totally disjoint ---
     ! --- and find out largest weight (required for pair thresholding)
     if (map2_present .and. do_mask2) then
        ...
        do kmask = 1, ncmask ! ncmask = 1 or 3 or 4
           ...
           product = 0.0_dp
           do i=0,npixtot-1
              tmp_prod = m_map(i+lb,k1) * m_map2(i+lb2,k2)
...
!              top_w8(kmask) =  max(top_w8(kmask),  tmp_prod)
     ...
     if (map_present) then
        ...
        if (degree_remove == 1 .or. degree_remove == 2) then
           if (megaverbose .and. degree_remove == 1) write(*,*) 'Subtract monopole from map'
           if (megaverbose .and. degree_remove == 2) write(*,*) 'Subtract monopole and dipole from map'
           call remove_dipole(nside, map_in(:,1), 1, degree_remove, lowmultipoles, zbounds, &
        ...
        if (do_mask) then
           ...
           do j=1,nmap
              kmask = min(j, nm_eff)
              ! do explicit loop because of bug in XLF compiler; EH, 2006-03-02
              do i=0,npixtot-1
                 map_in(i,j) = map_in(i,j) * m_map(i+lb, kmask)
              enddo
           enddo
        endif
     endif
     if (map2_present) then
        ...
        if (degree_remove == 1 .or. degree_remove == 2) then
           ...
           if (do_mask2) then 
              call remove_dipole(nside, map2_in(:,1), 1, degree_remove, lowmultipoles, zbounds, &
                   & mask = m_map2(:,1), weights = m_map2(:,1), silent=.not.megaverbose)
           else
              call remove_dipole(nside, map2_in(:,1), 1, degree_remove, lowmultipoles, zbounds, &
        ...
        if (do_mask2) then
           ...
           do j=1,nmap
              ...
              ! do explicit loop because of bug in XLF compiler; EH, 2006-03-02
              do i=0,npixtot-1
                 map2_in(i,j) = map2_in(i,j) * m_map2(i+lb, kmask2)
              enddo
           enddo
        endif
     endif
     ...
     if (map_present) then
        ...
        if (map2_present) then ! cross-correlate 2 maps
           call do_cl_with_alm(nside,npixtot,nlmax,map_in, cl, &
                &                        megaverbose,'(mapXmap)',nmap,ncor,polarization, &
                &                        map2=map2_in, cl_auto1=cl_auto1, cl_auto2=cl_auto2, symmetric=symmetric_cl)
     ...
     if (clmaskinput) then
        ...
        if (map2_present) then ! 2 maps -> assume the 2 masks to be different
           ...
           call do_cl_with_alm(nside, npixtot, nlmax, m_map, cl_mask, &
                &              megaverbose,message,nm_eff,ncmask,.false.,&
                &              map2=m_map2,symmetric=symmetric_cl)
        ...
        if (do_cov) then
           if (weights_present) then
              ...
              allocate(m_map_sq (0:npixtot-1,1:nm_eff))
              m_map_sq  = m_map**2
              if (map2_present) then ! 2 maps -> assume the 2 masks to be different
                 allocate(m_map2_sq(0:npixtot-1,1:nm_eff))
                 m_map2_sq = m_map2**2
                 message = '(weights^2Xweights^2)'
                 call do_cl_with_alm(nside, npixtot, nlmax, m_map_sq, cl_mask_sq, &
                      &              megaverbose,message,nm_eff,ncmask,.false.,&
                      &              map2=m_map2_sq,symmetric=symmetric_cl)
...
!        print*,'cumul done'
 ...
 &                               megaverbose,'(final result)',ncor,polarization) 
     ...
     deallocate(m_map)
     if (allocated(m_map2)) deallocate(m_map2)
